...
public class ModelReflector<M extends Model> {
    ...
    public Set<String> getAllowedValues(String enumeratedField){
        Method getter = getFieldGetter(enumeratedField);
        Enumeration enumeration = getAnnotation(getter,Enumeration.class);
        if (enumeration == null){
            throw new RuntimeException("Not an enumerated field:" + enumeratedField);
        }
        return getAllowedValues(enumeration);
    }
            ...
            synchronized (this) {
                ...
                if (map == null) {
                    ...
                    for (Class<? extends Model> modelClass : reflector.getSiblingModelClasses(getModelClass())) {
                        ...
                        if (map.get(COLUMN_NAME.class) == null) {
                            map.put(COLUMN_NAME.class, ref.getAnnotation(getter, COLUMN_NAME.class));
                        }
                        if (map.get(COLUMN_SIZE.class) == null) {
                            map.put(COLUMN_SIZE.class, ref.getAnnotation(getter, COLUMN_SIZE.class));
                        }
                        if (map.get(DATA_TYPE.class) == null) {
                            map.put(DATA_TYPE.class, ref.getAnnotation(getter, DATA_TYPE.class));
                        }
                        if (map.get(DECIMAL_DIGITS.class) == null) {
                            map.put(DECIMAL_DIGITS.class, ref.getAnnotation(getter, DECIMAL_DIGITS.class));
                        }
                        if (map.get(IS_NULLABLE.class) == null) {
                            map.put(IS_NULLABLE.class, ref.getAnnotation(getter, IS_NULLABLE.class));
                        }
                        if (map.get(IS_AUTOINCREMENT.class) == null) {
                            map.put(IS_AUTOINCREMENT.class, ref.getAnnotation(getter, IS_AUTOINCREMENT.class));
                        }
                        if (map.get(IS_VIRTUAL.class) == null) {
                            map.put(IS_VIRTUAL.class, ref.getAnnotation(getter, IS_VIRTUAL.class));
                        }
                        if (map.get(COLUMN_DEF.class) == null) {
                            map.put(COLUMN_DEF.class, ref.getAnnotation(getter, COLUMN_DEF.class));
                        }
                        if (map.get(ENCRYPTED.class) == null) {
                            map.put(ENCRYPTED.class, ref.getAnnotation(getter, ENCRYPTED.class));
                        }
                    }
    ...
    public List<String> getReferenceFields(Class<? extends Model> referredModelClass) {
        Timer timer = cat.startTimer();
        try {
            List<String> names = new SequenceSet<>();
    ...
    public List<String> getReferenceFields() {
        ...
        try {
            List<String> names = new SequenceSet<>();
            for (Method referredModelGetter : getReferredModelGetters()) {
                names.add(getReferenceField(referredModelGetter));
            }
            return names;
        } finally {
