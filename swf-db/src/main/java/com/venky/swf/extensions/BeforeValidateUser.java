	...
	public void beforeValidate(User model) {
		if (!ObjectUtil.isVoid(model.getName())) {
			...
			if (model.getRawRecord().isNewRecord()){
				List<User> userList = new Select().from(User.class).where(new Expression(ModelReflector.instance(User.class).getPool(),"lower(NAME)", Operator.EQ, name.toLowerCase())).execute();
				if (!userList.isEmpty()){
					if (name.indexOf('@') > 0){
