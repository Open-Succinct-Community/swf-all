...
public class ModelEditView<M extends Model> extends AbstractModelView<M> {
    ...
    protected boolean isFieldEditable(String fieldName){
        return !UIModelHelper.getDefaultHiddenFields(modelClass).contains(fieldName) && !UIModelHelper.getDefaultProtectedFields(modelClass).contains(fieldName) && !getReflector().getVirtualFields().contains(fieldName);
    }
}
